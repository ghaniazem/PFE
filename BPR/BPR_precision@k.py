...
# Dictionnaire pour stocker les valeurs de précision à k
...
for epoch in range(num_epochs):
    ...
    # Calculer la précision à k pour chaque utilisateur
    for k in k_values:
        precision_at_k_list = []
        for user_id in test_data['user_id'].unique():
            ...
            precision = precision_score(labels, [1 if i in recommended_movies else 0 for i in np.arange(num_items)])
            precision_at_k_list.append(precision)

        # Moyenne de la précision à k sur tous les utilisateurs
        mean_precision_at_k = np.mean(precision_at_k_list)
        precision_values[k].append(mean_precision_at_k)

    print("Epoch:", epoch + 1, "completed")

# Affichage des résultats
for k in k_values:
    plt.plot(range(num_epochs), precision_values[k], label=f'k={k}')
...
plt.ylabel('Précision@k')

plt.title('Précision@k en fonction du nombre d\'epochs')
plt.legend()
